...
- evitar perda de informação por conversão prematura para `uint8`.
...
Um kernel é uma pequena matriz de coeficientes, normalmente de tamanho ímpar, como $3 \times 3$, $5 \times 5$ ou $7 \times 7$.

Exemplo de kernel de média $3 \times 3$:
...
> [!WARNING]
> **Maior não significa melhor.** Aumentar o kernel reduz detalhes junto com o ruído. O parâmetro deve ser compatível com o tamanho das estruturas que você deseja preservar.
...
A mediana é 10. O valor extremo 255 praticamente não influencia o resultado.
...
> [!CAUTION]
> **Converter cedo demais perde informação.** Se você força a derivada negativa para `uint8` antes de tratar seu sinal, pode apagar parte relevante do gradiente.
...
Crie manualmente kernels de média $3 \times 3$, $5 \times 5$ e $9 \times 9$. Compare o efeito.
